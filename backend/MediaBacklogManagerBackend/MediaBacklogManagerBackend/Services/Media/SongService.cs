...
namespace MediaBacklogManagerBackend.Services.Media
{
    public class SongService : MediaService<Song>
    {
        ...
        internal async Task<Song?> CreateSong(CreateSongDto songDto, string userId)
        {
            Console.WriteLine($"Creating Song: {songDto.Title}");

            var song = await CheckExistsAsync(songDto.Title, songDto.ReleaseDate);

            if (song != null)
                return song;

            song = await MapSongCreation(songDto);

            return await CreateAsync(song, userId);
        ...
        {
            var id = songDto.Id;
            var song = await GetItemByIdAsync(id);
